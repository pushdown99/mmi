...
int
msleep(int tv_sec,int tv_usec)
{
    struct timeval timeout;

    timeout.tv_sec = (tv_sec);
    timeout.tv_usec = (tv_usec % 1000000);
    return(select (0, 0, 0, 0, &timeout));
}

char *
makefmttm (struct tm *tp, unsigned int us,char *fmt, char *s)
{
    char buf[BUFSIZ];
    ...
    for (*s = 0; *fmt; fmt++) {
        if(*fmt=='%') {
            switch (*++fmt) {
            case 'Y':
                sprintf (buf, "%04d", tp->tm_year + 1900);
                break;
            case 'y':
                sprintf (buf, "%02d", (tp->tm_year + 1900) % 100);
                break;
            case 'M':
                sprintf (buf, "%02d", tp->tm_min);
                break;
            case 'm':
                sprintf (buf, "%02d", tp->tm_mon + 1);
                break;
            case 'D':
                sprintf (buf, "%02d", tp->tm_yday + 1);
                break;
            case 'd':
                sprintf (buf, "%02d", tp->tm_mday);
                break;
            case 'H':
                sprintf (buf, "%02d", tp->tm_hour);
                break;
            case 'S':
                sprintf (buf, "%02d", tp->tm_sec);
                break;
            case 't':
                sprintf (buf, "%06d", us);
                break;
...
#define GETS(s,p,l) do {    \
    memcpy(s,p,l);          \
    s[l]=0;                 \
} while(0)

int
gettp (struct tm* tp, char *fmt, char *s)
{
    char buf[BUFSIZ],*p;
    int ival;
    ...
    for (p = s; *fmt; fmt++) {
        if(*fmt=='%') {
            ...
            case 'Y':
                GETS(buf,p,4); p+=4;
                tp->tm_year = atoi(buf)-1900;
                break;
            case 'y':
                GETS(buf,p,2); p+=2;
                tp->tm_year = (2000+atoi(buf))-1900;
                break;
            case 'M':
                GETS(buf,p,2); p+=2;
                tp->tm_min = atoi(buf);
...
//	struct timeb t;
 ...
 //   localtime_r(&t.time, &tm);
    return (makefmttm(&tm,tv.tv_usec,fmt,s));
}

char*
gfmttm(time_t* tp,char* fmt,char *s)
{
    struct tm tm;
    ...
    localtime_r(tp, &tm);
    return (makefmttm(&tm,0,fmt,s));
}

time_t
rfmttm(char *fmt,char *s)
{
    struct tm tm;
...
int
settimeout(struct timeval *timeout,int sec,int usec)
{
    timeout->tv_sec = sec;
...
unsigned long
tsftotvu(unsigned long tsf)
{
    return  (tstoushi[(tsf >> 24) & 0xff]
...
unsigned long
tvutotsf(unsigned long tvu)
{
    return ustotslo[(tvu) & 0xff]
