...
function lookup_run_parts() {
    while read line ; do
        ...
        if [[ -z "${match}" ]] ; then
            ...
            cron_job_dir=$(echo  "${match}" | gawk '{print $NF}')
...
#{ "min(s)": "25", "hours(s)": "6", "day(s)": "*", "month": "*", "weekday": "*", "user": "root", "command": " /etc/cron.daily/0anacron " }

cat "${temp}" \
| gawk 'BEGIN {printf "\n\n%-10s%-10s%-10s%-10s%-10s%-10s%-10s\n--------------------------------------------------------------------------------\n", \
  "MIN(s)", "HOURS(s)", "DAYS(s)", "MONTH", "WEEKDAY", "USER", "COMMAND"} \
  {printf "%-10s%-10s%-10s%-10s%-10s%-10s", $1, $2, $3, $4, $5, $6} \
